...
def eval_all(path_expected, path, column_expected='qaoa', csv_name_expected='parameter_map.csv',
    ...
    draw_chart(f'{path}/qaoa_execution', add_sim=True)
...
def boxplot_df(df, x='algorithm', y='value', col='graph_size', order=None, ylabel=None, xlabel=None,
               median_fontsize=14, context=None, xlabel_rotation=None, labelpad=20, color=None):
    with sns.plotting_context(context):
        bp = sns.catplot(df, x=x, y=y, order=order, kind='box', col=col, height=5, color=color)
