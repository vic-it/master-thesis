...
def get_qaoa_execution(path, sim=False):
    ...
    while True:
        try:
            ...
            break
    qaoa = 'qaoa' if not sim else 'sim-qaoa'
    qaoa_short = 'qaoa-short' if not sim else 'sim-qaoa-short'
    cut_qaoa = 'cut-qaoa' if not sim else 'sim-cut-qaoa'
    result = 'result' if not sim else 'sim-result'

    for item in execution_list:
        shots = sum(item[qaoa]['counts'].values())
        item[qaoa]['counts'] = {k: v / shots for k, v in item[qaoa]['counts'].items()}
        shots = sum(item[cut_qaoa]['counts'])
        item[cut_qaoa]['counts'] = [v / shots for v in item[cut_qaoa]['counts']]
        if qaoa_short in item.keys():
            shots = sum(item[qaoa_short]['counts'].values())
            item[qaoa_short]['counts'] = {k: v / shots for k, v in item[qaoa_short]['counts'].items()}
    return data['config'], data['graph'], data.get(result), execution_list


def get_qaoa_executions(path=None, min_exp=None, max_exp=None, config=None, sim=False):
    if path is None:
    ...
    for exp_dir in path.iterdir():
        ...
        for run_dir in exp_dir.iterdir():
            ...
            if sim:
                sim_qaoa_execution_path = run_dir / 'qaoa_execution_sim'
                _, _, d_result_sim, d_execution_sim_list = get_qaoa_execution(str(sim_qaoa_execution_path.resolve()),
                                                                              sim=True)
                for item, item_sim in zip(d_execution_list, d_execution_sim_list):
                    for key, value in item_sim.items():
                        item[key] = value

                for key, value in d_result_sim.items():
                    if key in ['__final_result__', 'params']:
                        continue
                    d_result['sim_' + key] = value
...
def draw_chart(path, n_largest=None, include_random=True, normalize=True, add_sim=False):
    sim = False
    if path.endswith('_sim'):
        sim = True
    config, graph, result, execution_list = get_qaoa_execution(path, sim=sim)
    if add_sim:
        sim_path = Path(path).parent
        sim_path = sim_path / 'qaoa_execution_sim'
        _, _, _, execution_sim_list = get_qaoa_execution(str(sim_path.resolve()), sim=True)
        for item, item_sim in zip(execution_list, execution_sim_list):
            for key, value in item_sim.items():
                item[key] = value
...
def get_appr_ratio_df(path=None, min_exp=None, max_exp=None, config=None, include_random=True, sim=False):
    execution_dict = get_qaoa_executions(path, min_exp, max_exp, config, sim)
    df_list = []
    for key, val in execution_dict.items():
        print(key)
        graph = val['graph']
