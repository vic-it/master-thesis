...
This repository contains the prototypical implementation used to generate results presented in our publication *"Investigating the effect of circuit cutting in QAOA for the MaxCut problem on NISQ devices"*.
...
Due to their large file sizes, the generated experiment data is uploaded separately:
 - `experiment_complete.zip` contains the raw and partly preprocessed experiment data. The unzipped data has about 100GB.
 - `calibration_data.zip` contains the calibration data of the quantum devices.
...
The classical simulation of the second experiment can be conducted with [experiment_2_sim.py](experiment_2_sim.py).
...
```
3. Postprocess the results with `load_data_from_executor` from [eval_param_maps.py](eval_param_maps.py):
```python
load_data_from_executor("path_to_result_directory_from_step_2", range(250, 750, 250))
```
4. Use `experiment_1_without_shot_memory.py` to compute the objective function of the graphs with a local simulator.
```
python experiment_1_without_shot_memory.py -b aer_simulator --steps 20 --graph-path <path to the graph of step 2.>
```
5. Postprocess the results with `load_data` from [eval_param_maps.py](eval_param_maps.py)
```python
load_data("path_to_simulator_result_directory_from_step_4")
```
6. Create a new folder that contains the two generated folders with the results from steps 2. and 4.
7. Add this folder to the already existing data
8. Run `eval_all_dirs` from [experiment_complete_eval.py](experiment_complete_eval.py) to generate plots for the new data
```python
eval_all_dirs(path="path_to_folder_from_step_6")
```
9. Rerun [notebooks/figures.ipynb](notebooks/figures.ipynb) to add the new data to the existing plots
